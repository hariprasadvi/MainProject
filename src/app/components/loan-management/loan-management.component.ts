...
export class LoanManagementComponent implements OnInit {
  ...
  getStatusTranslation(status: LoanStatus): string {
    const statusTranslations: { [key in LoanStatus]?: string } = {
      [LoanStatus.PENDING]: this.translations().PENDING,
      [LoanStatus.APPROVED]: this.translations().APPROVED,
      [LoanStatus.REJECTED]: this.translations().REJECTED
    };
    return statusTranslations[status] || status;
  }

  // New methods for the additional features in HTML
  calculateEMI(): number {
